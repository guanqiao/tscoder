...
describe("Package Manager registry configuration", () => {
  ...
  test("should use default registry resolution", async () => {
    // Read the bun/index.ts file
    const bunIndexPath = path.join(__dirname, "../src/bun/index.ts")
    const content = await fs.readFile(bunIndexPath, "utf-8")

    // Verify that it uses default resolution
    expect(content).toContain("default registry resolution")
    expect(content).toContain("use them automatically")
