...
import { which, file, spawnAsync, streamToText } from "@/platform"
...
export const prettier: Info = {
  ...
  async enabled() {
    ...
    for (const item of items) {
      const json = await file(item).json()
    ...
    for (const config of configs) {
      ...
      if (found.length > 0) {
        if (config === "pyproject.toml") {
          const content = await file(found[0]).text()
...
export const rlang: Info = {
  ...
  async enabled() {
    ...
    try {
      const proc = spawnAsync(["air", "--help"], {
        stdout: "pipe",
        stderr: "pipe",
      })
      await proc.exited
...
export const uvformat: Info = {
  ...
  async enabled() {
    ...
    if (which("uv") !== null) {
      const proc = spawnAsync(["uv", "format", "--help"], { stderr: "pipe", stdout: "pipe" })
