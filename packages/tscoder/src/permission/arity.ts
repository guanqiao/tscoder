...
This dictionary is used to identify the "human-understandable command" from an input shell command.### **RULES (follow strictly)**1. Each entry maps a **command prefix string â†?number**, representing how many **tokens** define the command.
...
6. Include the **most commonly used commands** across many stacks and languages. More is better.### **Semantics examples*** `touch foo.txt` â†?`touch` (arity 1, explicitly listed)
* `git checkout main` â†?`git checkout` (because `git` has arity 2)
* `npm install` â†?`npm install` (because `npm` has arity 2)
* `npm run dev` â†?`npm run dev` (because `npm run` has arity 3)
* `python script.py` â†?`python script.py` (default: whole input, not in dictionary)### **Now generate the dictionary.**
*/
