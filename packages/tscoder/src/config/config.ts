...
import { file, writeFile, Glob } from "@/platform"
...
export namespace Config {
  ...
  export const state = Instance.state(async () => {
    ...
    // 1) Local well-known configs (~/.config/opencode/wellknown/*.json)
    ...
    // Load local well-known configs from ~/.config/opencode/wellknown/
    const wellknownDir = path.join(Global.Path.config, "wellknown")
    if (existsSync(wellknownDir)) {
      const files = await fs.readdir(wellknownDir).catch(() => [])
      for (const file of files) {
        if (file.endsWith(".json")) {
          const filepath = path.join(wellknownDir, file)
          log.debug("loading local well-known config", { path: filepath })
          const config = await loadFile(filepath)
          if (Object.keys(config).length > 0) {
            result = mergeConfigConcatArrays(result, config)
            log.debug("loaded local well-known config", { file })
          }
        }
      }
    }
  ...
  export async function installDependencies(dir: string) {
    ...
    const json = await file(pkg)
      .json()
      .catch(() => ({}))
    json.dependencies = {
      ...json.dependencies,
      "@tscoder/plugin": targetVersion,
    }
    await writeFile(pkg, JSON.stringify(json, null, 2))
    await new Promise((resolve) => setTimeout(resolve, 3000))

    const gitignore = path.join(dir, ".gitignore")
    const hasGitIgnore = await file(gitignore).exists()
  ...
  async function needsInstall(dir: string) {
    ...
    const pkg = path.join(dir, "package.json")
    const pkgF = file(pkg)
    const pkgExists = await pkgF.exists()
    if (!pkgExists) return true

    const parsed = await pkgF.json().catch(() => null)
    const dependencies = parsed?.dependencies ?? {}
    const depVersion = dependencies["@tscoder/plugin"]
    if (!depVersion) return true

    const targetVersion = Installation.isLocal() ? "latest" : Installation.VERSION
    if (targetVersion === "latest") {
      const isOutdated = await PackageRegistry.isOutdated("@tscoder/plugin", depVersion, dir)
  ...
  export async function updateGlobal(config: Info) {
    ...
    const before = await file(filepath)
      .text()
      .catch((err) => {
        if (err.code === "ENOENT") return "{}"
        throw new JsonError({ path: filepath }, { cause: err })
