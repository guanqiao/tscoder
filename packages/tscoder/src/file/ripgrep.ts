...
import { $, file, writeFile, spawnAsync, streamToText, which } from "@/platform"
...
export namespace Ripgrep {
      ...
      if (config.extension === "tar.gz") {
        ...
        if (proc.exitCode !== 0)
          throw new ExtractionFailedError({
            filepath,
            stderr: await streamToText(proc.stderr!),
          })
      }
      if (config.extension === "zip") {
        const zipFileReader = new ZipReader(new BlobReader(new Blob([await file(archivePath).arrayBuffer()])))
        ...
        await writeFile(filepath, Buffer.from(await rgBlob.arrayBuffer()))
  ...
  export async function* files(input: {
    ...
    // spawn should throw this, but it incorrectly reports that the executable does not exist.
