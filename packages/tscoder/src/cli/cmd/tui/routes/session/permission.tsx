...
import type { PermissionRequest } from "@tscoder/sdk/v2"
              ...
              body={
                <Switch>
                  ...
                  <Match when={props.request.permission === "read"}>
                    <TextBody icon="â†? title={`Read ` + normalizePath(input().filePath as string)} />
                  </Match>
                  <Match when={props.request.permission === "glob"}>
                    <TextBody icon="âœ? title={`Glob "` + (input().pattern ?? "") + `"`} />
                  </Match>
                  <Match when={props.request.permission === "grep"}>
                    <TextBody icon="âœ? title={`Grep "` + (input().pattern ?? "") + `"`} />
                  </Match>
                  <Match when={props.request.permission === "list"}>
                    <TextBody icon="â†? title={`List ` + normalizePath(input().path as string)} />
                  </Match>
                  ...
                  <Match when={props.request.permission === "task"}>
                    <TextBody
                      ...
                      description={"â—?" + input().description}
                    />
                  </Match>
                  ...
                  <Match when={props.request.permission === "websearch"}>
                    <TextBody icon="â—? title={`Exa Web Search "` + (input().query ?? "") + `"`} />
                  </Match>
                  <Match when={props.request.permission === "codesearch"}>
                    <TextBody icon="â—? title={`Exa Code Search "` + (input().query ?? "") + `"`} />
                  </Match>
                  <Match when={props.request.permission === "external_directory"}>
                    {(() => {
                      ...
                      return <TextBody icon="â†? title={`Access external directory ` + dir} />
                    })()}
                  </Match>
                  <Match when={props.request.permission === "doom_loop"}>
                    <TextBody icon="âŸ? title="Continue after repeated failures" />
                  </Match>
                  <Match when={true}>
                    <TextBody icon="âš? title={`Call tool ` + props.request.permission} />
                  </Match>
                </Switch>
              }
